   ...
   public class Tracker implements GoogleAnalyticsAPI
   {
      ...
      public function trackTrans() : void
      {
         ...
         if(this._takeSample())
         {
            ...
            {
               _loc3_ = this._ecom.getTransFromArray(_loc1_);
               _loc4_.push(_loc3_.toGifParams());
               _loc2_ = 0;
               while(_loc2_ < _loc3_.getItemsLength())
               {
                  _loc4_.push(_loc3_.getItemFromArray(_loc2_).toGifParams());
                  _loc2_++;
               }
               _loc1_++;
            }
            ...
            {
               this._gifRequest.send(this._account,_loc4_[_loc1_]);
               _loc1_++;
            }
         }
      }
      ...
      private function _formatReferrer() : String
      {
         ...
         var _loc2_:URL = null;
         var _loc3_:URL = null;
         var _loc4_:String = this._info.referrer;
         if(_loc4_ == "" || _loc4_ == "localhost")
         {
            _loc4_ = "-";
         }
         else
         {
            ...
            _loc2_ = new URL(_loc4_);
            _loc3_ = new URL("http://" + _loc1_);
            if(_loc2_.hostName == _loc1_)
            {
               return "-";
            }
            if(_loc3_.domain == _loc2_.domain)
            {
               if(_loc3_.subDomain != _loc2_.subDomain)
               {
                  _loc4_ = "0";
               }
            }
         ...
         this._debug.info("formated referrer: " + _loc4_,VisualDebugMode.advanced);
         return _loc4_;
      }
      ...
      {
         var _loc2_:Variables = null;
         var _loc3_:Variables = new Variables();
         _loc3_.URIencode = true;
         var _loc4_:DocumentInfo = new DocumentInfo(this._config,this._info,this._formatedReferrer,param1,this._adSense);
         this._debug.info("docInfo: " + _loc4_.toURLString(),VisualDebugMode.geek);
         if(this._config.campaignTracking)
         {
            _loc2_ = this._campaignInfo.toVariables();
         }
         var _loc5_:Variables = this._browserInfo.toVariables();
         _loc3_.join(_loc4_.toVariables(),_loc5_,_loc2_);
         return _loc3_;
      }
         ...
         if(this._adSense.gaGlobal && this._adSense.dh == String(this._domainHash))
         {
            if(this._adSense.sid)
            {
               ...
               if(this._debug.verbose)
               {
                  ...
                  _loc1_ = _loc1_ + ("Override currentTime(" + this._buffer.utma.currentTime + ") from AdSense sid(" + Number(this._adSense.sid) + ")");
                  this._debug.info(_loc1_,VisualDebugMode.geek);
               }
            }
            if(this._isNewVisitor)
            {
               if(this._adSense.sid)
               {
                  ...
                  if(this._debug.verbose)
                  {
                     ...
                     _loc2_ = _loc2_ + ("Override lastTime(" + this._buffer.utma.lastTime + ") from AdSense sid(" + Number(this._adSense.sid) + ")");
                     this._debug.info(_loc2_,VisualDebugMode.geek);
                  }
               }
               if(this._adSense.vid)
               {
                  ...
                  if(this._debug.verbose)
                  {
                     ...
                     _loc4_ = _loc4_ + ("Override sessionId(" + this._buffer.utma.sessionId + ") from AdSense vid(" + Number(_loc3_[0]) + ")\n");
                     _loc4_ = _loc4_ + ("Override firstTime(" + this._buffer.utma.firstTime + ") from AdSense vid(" + Number(_loc3_[1]) + ")");
