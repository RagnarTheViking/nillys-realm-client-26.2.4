package com.google.analytics.core
{
   ...
   public class GIFRequest
   {
      ...
      public function updateToken() : void
      {
         ...
         _loc1_ = (_loc2_ - this._buffer.utmb.lastTime) * (this._config.tokenRate / 1000);
         if(this._debug.verbose)
         {
            this._debug.info("tokenDelta: " + _loc1_,VisualDebugMode.geek);
         }
         if(_loc1_ >= 1)
         {
            this._buffer.utmb.token = Math.min(Math.floor(this._buffer.utmb.token + _loc1_),this._config.bucketCapacity);
            this._buffer.utmb.lastTime = _loc2_;
            if(this._debug.verbose)
            {
               this._debug.info(this._buffer.utmb.toString(),VisualDebugMode.geek);
            }
      ...
      private function _debugSend(param1:URLRequest) : void
      {
         ...
         switch(this._debug.mode)
         {
            case VisualDebugMode.geek:
               _loc3_ = "Gif Request #" + this._alertcount + ":\n" + param1.url;
               break;
            case VisualDebugMode.advanced:
               ...
               _loc2_ = this._shortenURL(_loc2_);
               _loc3_ = "Send Gif Request #" + this._alertcount + ":\n" + _loc2_ + " ?";
               break;
            case VisualDebugMode.basic:
            default:
               _loc3_ = "Send " + this._config.serverMode.toString() + " Gif Request #" + this._alertcount + " ?";
         }
         this._debug.alertGifRequest(_loc3_,param1,this);
         this._alertcount++;
      }
